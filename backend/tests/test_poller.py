...
def test_razers_workday_confirmation_is_tracked(session):
    """Real mail: no display name, so the employer is only in the mailbox name."""
    from sqlmodel import select as _select
    note = process_message(session, ParsedMessage(
        "razer1", "t1", "razer@myworkday.com",
        "We've got your application for Product Developer Intern !",
        "Dear Shi Kai, We are glad to hear about your interest to join our global "
        "quest as Product Developer Intern .",
        None,
    ))
    company = session.exec(_select(Company)).one()
    assert company.name == "Razer"
    assert company.email_domains == ""          # myworkday.com is not Razer's
    apps = _apps(session)
    assert len(apps) == 1 and apps[0].status == AppStatus.confirmed
    assert session.get(Job, apps[0].job_id).title == "Product Developer Intern"
    assert note is not None and note.type == "confirmation"


def test_autodesks_workday_confirmation_is_filed_under_autodesk(session):
    """Real mail: it used to create a company called "AutoNotification workday"."""
    from sqlmodel import select as _select
    process_message(session, ParsedMessage(
        "adsk1", "t1", "AutoNotification workday <autodesk@myworkday.com>",
    ...
    company = session.exec(_select(Company)).one()
    assert company.name == "Autodesk"
    job = session.get(Job, _apps(session)[0].job_id)
    assert job.title == "Intern, Product Manager [PSET-Access-PM]"


def test_a_workday_tenant_finds_the_company_already_tracked(session):
    from sqlmodel import select as _select
    company, app = _seed_named(session, "Razer", "razer", "razer.com",
                               title="Product Developer Intern")
    process_message(session, ParsedMessage(
        "razer2", "t1", "razer@myworkday.com",
        "We've got your application for Product Developer Intern !", "", None,
    ))
    session.refresh(app)
    assert app.status == AppStatus.confirmed
    assert len(session.exec(_select(Company)).all()) == 1   # no second Razer
    assert len(_apps(session)) == 1


def test_a_workday_account_email_is_filed_not_tracked(session):
    """Real mail: the candidate-account check that came a few minutes earlier."""
    _company, app = _seed_named(session, "Razer", "razer", "razer.com")
    note = process_message(session, ParsedMessage(
        "razer0", "t1", "razer@otp.workday.com", "Verify your candidate account",
